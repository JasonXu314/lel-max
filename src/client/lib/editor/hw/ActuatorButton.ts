...
import { PhantomActuator } from './Actuator';

export class ActuatorButton extends Entity {
	...
	public init(renderEngine: RenderEngine, context: EngineContext): void {
		...
		context.engine.on('entityClicked', (entity) => {
			if (entity === this) {
				context.engine.activePanes[1].add(new PhantomActuator());
			}
		});
	}
	...
	public render(metadata: Metadata): void {
		...
		this.renderEngine.text(this.position, '+ Add Actuator', { color: 'white', fontSize: 15 });
	}
