...
export class ModulusValue extends Value implements IValueHost {
	...
	public duplicate(): Block[][] {
		...
		const [[that]] = super.duplicate() as [[ModulusValue]];
	...
	public compile(scope: LexicalScope): ExprCompileResult {
		...
		return {
			code: `${parenthesize(leftResult, OperatorPrecedence.MOD)} % ${parenthesize(rightResult, OperatorPrecedence.MOD)}`,
			meta: { requires: union(leftResult.meta.requires, rightResult.meta.requires), precedence: OperatorPrecedence.MOD }
		};
	}
}
