...
interface GTEPredicateShapeParams {
	width: number;
	height: number;
	angleInset: number;
}
...
export class GTEPredicate extends Predicate implements IValueHost {
	...
	public constructor() {
		...
		this.shape = new PathBuilder<GTEPredicateShapeParams>(
			({ width }) => width,
			({ height }) => height
		)
	...
	public duplicate(): Block[][] {
		...
		const [[that]] = super.duplicate() as [[GTEPredicate]];
	...
	public compile(scope: LexicalScope): ExprCompileResult {
		...
		return {
			code: `${parenthesize(leftResult, OperatorPrecedence.GTE)} >= ${parenthesize(rightResult, OperatorPrecedence.GTE)}`,
			meta: { requires: union(leftResult.meta.requires, rightResult.meta.requires), precedence: OperatorPrecedence.GTE }
		};
	}
}
