...
export class EqualityPredicate extends Predicate implements IValueHost {
	...
	public duplicate(): Block[][] {
		...
		const [[that]] = super.duplicate() as [[EqualityPredicate]];
	...
	public compile(scope: LexicalScope): ExprCompileResult {
		...
		return {
			code: `${parenthesize(leftResult, OperatorPrecedence.EQ)} == ${parenthesize(rightResult, OperatorPrecedence.EQ)}`,
			meta: { requires: union(leftResult.meta.requires, rightResult.meta.requires), precedence: OperatorPrecedence.EQ }
		};
	}
}
