...
interface LTEPredicateShapeParams {
	width: number;
	height: number;
	angleInset: number;
}

export class LTEPredicate extends Predicate implements IValueHost {
	...
	public constructor() {
		...
		this.shape = new PathBuilder<LTEPredicateShapeParams>(
			({ width }) => width,
			({ height }) => height
		)
	...
	public duplicate(): Block[][] {
		...
		const [[that]] = super.duplicate() as [[LTEPredicate]];
	...
	public compile(scope: LexicalScope): ExprCompileResult {
		...
		return {
			code: `${parenthesize(leftResult, OperatorPrecedence.LTE)} <= ${parenthesize(rightResult, OperatorPrecedence.LTE)}`,
			meta: { requires: union(leftResult.meta.requires, rightResult.meta.requires), precedence: OperatorPrecedence.LTE }
		};
	}
}
